...
session_start_checklist:
  - Read docs/PROGRESS.md for current phase, completed tasks, and next steps
  - Check docs/ERRORS.md for active blockers (ERROR-XXX with status "Open")
  - Review docs/DECISIONS.md for architectural decisions (DEC-XXX)
  - Scan docs/ERRORS.md "Known Patterns" to avoid repeating mistakes
  - Continue from last in-progress task in docs/PROGRESS.md

session_end_checklist:
  - Update docs/PROGRESS.md with completed tasks and current status
  - Log any new errors to docs/ERRORS.md (assign next ERROR-XXX ID)
  - Document architectural decisions in docs/DECISIONS.md (next DEC-XXX)
  - Update docs/TECH-STACK.md if new tools were added or evaluated

error_handling_protocol:
  - Capture full error message, context, and stack trace
  - Add to docs/ERRORS.md immediately with ERROR-XXX ID
  - If blocker: mark as Priority P0 in docs/ERRORS.md
...
general_rules:
  before_writing_code:
    - Check docs/ERRORS.md "Known Patterns" to avoid known issues
    - Verify tool/library exists in docs/TECH-STACK.md before adding new dependency
  ...
  file_organization: |
    ...
    │   ├── Cargo.toml
    │   └── src/
    ├── ue5-client/                 # Unreal Engine 5 project
    │   ├── TowerGame.uproject
    │   └── Source/
    ├── bevy-server/                # Bevy authoritative server
    │   ├── Cargo.toml
    │   └── src/
    ...
    ├── scripts/                    # Build/deploy scripts (.sh)
    ├── bugfix_engine/              # UE5 compiler bugfix scripts (.ps1, .bat)
    ...
    ├── logs/                       # All log files
    │   ├── bevy-server/            # Bevy server logs
    │   └── ue5-client/             # UE5 build logs
...
always_do:
  ...
  - Log errors to docs/ERRORS.md immediately
